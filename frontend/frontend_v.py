...
class RoverSimulation:
    def __init__(self):
        ...
        self.timer_period = 0.1  # Execute steps every 0.1 seconds
    ...
    def step(self):
        ...
        return state_name, self.x, self.y, self.theta, v
...
if "velocity_data" not in st.session_state:
    st.session_state.velocity_data = []
...
velocity_plot_placeholder = st.empty()
...
velocity_data = st.session_state.velocity_data
...
def update_plots():
    ...
    if velocity_data:
        ...
        plt.legend()
        velocity_plot_placeholder.pyplot(plt)

# Main simulation loop
if st.session_state.running:
    while st.session_state.running:
        if not st.session_state.paused:
            state_name, x, y, theta, velocity = st.session_state.simulation.step()
            output_placeholder.text(f"{state_name}\nPozycja: x={x:.2f}, y={y:.2f}, theta={theta:.2f}, prędkość={velocity:.2f}")
            ...
            time_data.append(st.session_state.simulation.current_time)
